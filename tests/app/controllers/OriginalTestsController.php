...
class OriginalTestsController extends Controller
{
    ...
    public function dbtestAction()
    {
        ...
        var_dump(
            (string)DbQuery::sql()->from(array('t' => 'test', 'aeee' => 'another'))->where(
                array('test', '=', 3, 'another')
            )
        );
        ...
        var_dump(
            (string)DbQuery::sql()->replace()->from('test')->fields(array('id' => 1, 'text' => 'test'))->where('id > 1')
        );
    }
}
